...
const StyledBlock = styled(
  (props: BlockProps & SuomifiThemeProp & GlobalMarginProps) => {
    const { theme, variant, globalMargins, ...passProps } = props;
    return <SemanticBlock variant={variant} {...passProps} />;
  },
)`
  ${({ theme, variant, globalMargins }) =>
    baseStyles(theme, variant, globalMargins?.block)}
`;
