...
<SearchInput
  ...
  onSuggestionFocused={(value) => console.log(value)}
  onBlur={(e) => {
...
### Search suggestions with controlled state
...
<SearchInput
  ...
  suggestions={suggestions}
  onSuggestionSelected={(id) => handleSuggestionSelection(id)}
  onSuggestionFocused={(label) => setVisibleValue(label)}
  onSuggestionListClosedByUser={() => setVisibleValue(inputValue)}
  onBlur={(e) => {
